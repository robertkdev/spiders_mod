...
public class GeoExampleEntity extends PathfinderMob implements IAnimatable, IAnimationTickable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        if (this.isAnimating) {
            event.getController()
                    .setAnimation(new AnimationBuilder().addAnimation("animation.bat.fly", ILoopType.EDefaultLoopTypes.PLAY_ONCE)
                            .addAnimation("animation.bat.idle", ILoopType.EDefaultLoopTypes.PLAY_ONCE));
        } else {
    ...
    private <E extends IAnimatable> PlayState predicateSpin(AnimationEvent<E> event) {
        event.getController()
                .setAnimation(new AnimationBuilder().addAnimation("animation.bat.spin", ILoopType.EDefaultLoopTypes.LOOP));
        return PlayState.CONTINUE;
    }
